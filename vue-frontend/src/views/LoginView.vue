  ...
  <div class="login-page">
    <div class="login-layout">
      ...
      <div class="login-left">
        <div class="brand">
          <span class="brand-logo brand-mark">SG</span>
          <span class="brand-name">ShariahGuard</span>
        </div>
        <div class="brand-content">
          <h2>샤리아·할랄 기준을<br>구독하는 컴플라이언스 콘솔</h2>
          <p>은행·증권사 컴플라이언스 담당자용 콘솔입니다.</p>
      ...
      <div class="login-right">
        <div class="login-box fade-in-up">
          ...
          <div v-if="!showRegister" class="section">
            ...
            <p class="section-desc">회사 계정으로 로그인합니다.</p>
          ...
          <div v-else class="section">
            ...
            <form @submit.prevent="handleRegister" class="form">
              ...
              <div class="form-group">
                <label class="form-label">금융회사명</label>
                <input v-model="registerForm.organizationName" type="text" class="form-input" placeholder="Skala Securities" required />
              </div>
              <div class="form-group">
              ...
              <div class="form-group">
                ...
                <select v-model="registerForm.role" class="form-input">
                  <option value="STUDENT">컴플라이언스 분석가</option>
                  <option value="INSTRUCTOR">금융회사 관리자</option>
                </select>
              </div>
...
const registerForm = ref({ name: '', organizationName: '', email: '', password: '', role: 'STUDENT' })

const features = ['샤리아·할랄 모델 구독 관리', '종목·주문 API 연동', '판정 근거·감사 이력 보존']
...
async function handleRegister() {
  ...
  try {
    ...
    const issuedApiKey = response.data?.data?.apiKey
    success.value = issuedApiKey
      ? `회원가입 완료. 최초 API 키: ${issuedApiKey} (지금 안전하게 보관하세요)`
      : '회원가입 완료! 로그인 페이지로 이동합니다.'
    registerForm.value = { name: '', organizationName: '', email: '', password: '', role: 'STUDENT' }
    if (!issuedApiKey) {
      setTimeout(() => {
...
.brand-mark { display: grid; place-items: center; color: #185f46; background: #fff; font-size: 12px; font-weight: 800; }
